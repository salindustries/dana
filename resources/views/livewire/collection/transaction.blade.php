    ...
    <div class="col-12">
      <div class="card">
        ...
        <x-table>
          ...
          <x-slot name="head">
            <x-table.heading sortable wire:click="sortBy('id')" :direction="$sortField === 'id' ? $sortDirection : null"> ID
            </x-table.heading>
            <x-table.heading> Photo
            </x-table.heading>
            <x-table.heading sortable wire:click="sortBy('name')" :direction="$sortField === 'name' ? $sortDirection : null"> Name
            </x-table.heading>
            <x-table.heading sortable wire:click="sortBy('email')" :direction="$sortField === 'email' ? $sortDirection : null">Email
            </x-table.heading>
            <x-table.heading sortable wire:click="sortBy('role_id')" :direction="$sortField === 'role' ? $sortDirection : null">Role
            </x-table.heading>
            ...
            @foreach ($users as $user)
              <x-table.row wire:key="row-{{ $user->id }}">
                ...
                <x-table.cell class="avatar avatar-xl position-relative">
                  @if ($user->picture)
                    <img src="/storage/{{ $user->picture }} " alt="picture" class="w-100 rounded-circle shadow-sm">
                  @else
